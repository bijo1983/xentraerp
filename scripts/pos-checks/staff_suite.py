...
try:
    ...
    r = pos.create_pos_user("zz.new@example.com", "ZZ New Cashier", "482913", "ZZ S Reg")
    ...
    check("cashier cannot list staff / PINs", err(pos.list_pos_users, "not permitted"))
    check("cashier cannot create staff", err(lambda: pos.create_pos_user("zz.x@example.com", "X", "111222"), "not permitted"))
    check("cashier cannot deactivate a PIN", err(lambda: pos.set_pin_active(u, 0), "not permitted"))
