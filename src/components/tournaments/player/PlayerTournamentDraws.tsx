...
export const PlayerTournamentDraws: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-[50vh] flex items-center justify-center">
        <div className="text-text-secondary text-sm">Loading draws...</div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-3xl mx-auto">
        <div className="rounded-lg border border-dashed border-background-subtle bg-background p-6 text-center text-sm text-text-secondary">
          Register for a tournament to view the official draw once it is published.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-text-primary">Tournament Draws</h1>
        <p className="text-sm text-text-secondary mt-1">Follow brackets for each category you have entered.</p>
      </div>
        ...
        <div className="rounded-xl border border-background-subtle bg-background shadow-sm p-4">
          {selectedEntry ? (
            <>
              ...
              <BracketView eventId={selectedEntry.event.id} eventType={discipline} />
            </>
          ) : (
            <div className="text-sm text-text-secondary">Select a category to view its draw.</div>
          )}
        </div>
      </div>
    </div>
...
export default PlayerTournamentDraws;
