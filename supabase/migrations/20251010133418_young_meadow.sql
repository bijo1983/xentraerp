...
CREATE TABLE IF NOT EXISTS court_peak_hours (
  ...
  name text NOT NULL, -- e.g., "Evening Rush", "Weekend Premium"
  ...
  pricing_value decimal(10,2) NOT NULL, -- multiplier (e.g., 1.5) or fixed rate
...
BEGIN
  ...
  FOR peak_hour_record IN
    ...
    WHERE court_id = p_court_id
      ...
      AND (
        -- Slot overlaps with peak hour period
        (p_start_time >= start_time AND p_start_time < end_time) OR
        (p_end_time > start_time AND p_end_time <= end_time) OR
        (p_start_time <= start_time AND p_end_time >= end_time)
      )
    ORDER BY pricing_value DESC -- Use highest rate if multiple peak periods overlap
    LIMIT 1
  LOOP
    IF peak_hour_record.pricing_type = 'fixed' THEN
      peak_fixed_rate := peak_hour_record.pricing_value;
      EXIT; -- Use fixed rate, no need to check further
    ELSE
