/*
  ...
  2. Functions
    ...
    - `sync_auth_user_profile()` - Update profile tracking
...
DROP POLICY IF EXISTS "Users can read own profile info" ON auth_user_profiles;
CREATE POLICY "Users can read own profile info"
  ON auth_user_profiles FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own profile info" ON auth_user_profiles;
CREATE POLICY "Users can update own profile info"
  ON auth_user_profiles FOR UPDATE
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "System can insert profile info" ON auth_user_profiles;
