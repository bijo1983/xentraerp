...
export default function NewSalesOrderPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setCustomers(cust || []);
      setItems(itm || []);
    })().catch(() => setError('Failed to load customers/items from ERPNext.'));
  }, []);
  ...
  const handleSubmit = async (submitAfter: boolean) => {
    ...
    try {
      ...
      const doc = await frappe.createDoc('Sales Order', payload);
      if (submitAfter && doc?.name) {
        await frappe.submitDoc('Sales Order', doc.name);
      }
      router.push('/sales');
    } catch (err: unknown) {
      const message =
        (err as { response?: { data?: { exception?: string; _server_messages?: string } } })?.response
          ?.data?.exception ||
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={defaultsLoading ? 'Loading…' : currency || '—'}
              disabled
              readOnly
            />
          </div>
